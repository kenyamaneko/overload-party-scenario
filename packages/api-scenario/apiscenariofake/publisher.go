...
// Publisher は送信側サービス (scenario) のテスト用 fake。Broker 経由で全 subscriber に
// 配信しつつ、自身の Published() スライスに発行記録を残す。scenario 自身の境界
// テストで「意図した topic と payload を発行したか」を検証する用途に加え、
// consumer 側テストから「scenario がこういう publish をしたとき受信側はどう動くか」
// を再現する用途にも使う。
