// Package apiscenariofake は api-scenario の送信側サービス (scenario) が
// consumer 側サービスに提供するテスト用 fake。送信側パッケージがテスト
...
//   - Publisher: 送信側サービス (scenario) の自テストが発行アサーションに使う fake
//   - Subscriber: 受信側サービス (account / card / gateway 等) のテストで消費検証に使う fake
//   - typed helpers: TopicPlayerOnboarded に対応する型付き publish/receive API (typed.go)
//
...
// Broker は apiscenariofake 内の publisher / subscriber を仲介する topic ベースの
