...
1. 依頼から type を判定し、タイトルを生成する
   - type / タイトル形式は [CLAUDE.md](../../CLAUDE.md) の「ブランチ・Issue 運用」に従う
   - 例: 「新しいエピソードを追加したい」→ `feat` / 「script 配信ロジックを直す」→ `fix` / 「アンロック条件を変更」→ `feat` or `refactor` / 「ファクション別スクリプトの言語フォールバックを修正」→ `fix`
2. 本文を下記テンプレに沿って書く
...
- なぜやるか(背景)
- 何ができたら完了か(受け入れ基準)
- 対象エピソード / script / ファクション / 言語が特定できるか
- `fix` の場合: 再現手順、発見環境(dev / stg / prod)、対象 GCS オブジェクトや Pub/Sub トピックの情報
