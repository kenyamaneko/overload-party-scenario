...
func TestClaimUnpublished(t *testing.T) {
	...
	t.Run("未配信イベントの取得", func(t *testing.T) {
		...
		tests := []struct {
			...
			{
				name: "failure_countが閾値の1つ手前の行があるとき、取得する",
				seeds: []seed{
		...
		t.Run("取得成功後、last_attempted_atがnow()に更新される", func(t *testing.T) {
			...
			require.NoError(t, sharedPg.Pool.QueryRow(ctx,
				`SELECT last_attempted_at IS NOT NULL FROM scenario.outbox_events WHERE event_id = $1`,
	...
	t.Run("配信済みマーク", func(t *testing.T) {
		...
		for i, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				require.NoError(t, sharedPg.Pool.QueryRow(ctx,
					`SELECT published_at IS NOT NULL, last_error FROM scenario.outbox_events WHERE event_id = $1`,
	...
	t.Run("失敗の記録", func(t *testing.T) {
		...
		for i, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				...
				var publishedAtNotNull bool
				require.NoError(t, sharedPg.Pool.QueryRow(ctx,
					`SELECT failure_count, last_error, published_at IS NOT NULL FROM scenario.outbox_events WHERE event_id = $1`,
